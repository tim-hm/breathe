...
/// lines including `DrawnStroke` — `OndStyle` could hold a shared renderer,
/// since it may depend on both `OndKit` and `OndUI`, so this is a choice
...
/// difference. What must not diverge cannot: `TechniqueDrawing` in `OndKit`
